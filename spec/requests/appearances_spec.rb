...
RSpec.describe "/dashboard/appearance", type: :request do
  ...
  describe "POST /dashboard/appearance" do
    ...
    it "renders an warning message if the pamas is missing a required field" do
  ...
  describe "PUT /dashboard/appearance/1" do
    ...
    it "updates the existing entity with valid params" do
      put "/dashboard/appearance/2", params: {appearance: {profile_title: "Amazing Coder", bio: "Nice to meet you!", user_id: @test_user.id, bg_color: "green"}}
      expect(response.body).to include("Amazing Coder")
    end

    it "updates only edited fields" do
      put "/dashboard/appearance/3", params: {appearance: {profile_title: "Happy Coder", user_id: @test_user.id, picture_url: "https://abcd.com"}}
